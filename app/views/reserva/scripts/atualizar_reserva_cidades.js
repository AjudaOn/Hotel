document.addEventListener('DOMContentLoaded', function() {
    ...
    if (ufSelect && cidadeSelect) {
                    ...
                    if (xhr.status === 200) {
                        try {
                            ...
                            if (data && data.length > 0) {
                                ...
                                // Se houver uma cidade previamente selecionada, selecione-a novamente
                                if (cidadeSelect.dataset.selected) {
                                    const selectedCidade = cidadeSelect.dataset.selected;
                                    for (let i = 0; i < cidadeSelect.options.length; i++) {
                                        if (cidadeSelect.options[i].value === selectedCidade) {
        ...
        // Se já tiver uma UF selecionada ao carregar a página, carregar as cidades
        if (ufSelect.value) {
            // Armazenar a cidade selecionada atualmente (se houver)
            if (cidadeSelect.value) {
                cidadeSelect.dataset.selected = cidadeSelect.value;
            }
