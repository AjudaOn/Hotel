...
$reserva9_id = isset($_GET['id']) ? $_GET['id'] : (isset($_SESSION['reserva_id']) ? $_SESSION['reserva_id'] : null);
...
$stmtCompleta = $db->prepare($reserva9);
$stmtCompleta->bind_param("i", $reserva9_id);
...
echo "<pre>Query executada com reserva_id = {$reserva9_id}</pre>";
echo "<h4>Dados da Reserva:</h4>";
echo "<pre>";
print_r($reserva9);
echo "</pre>";
echo "<h4>Acompanhantes encontrados: " . count($acompanhantes9) . "</h4>";
echo "<pre>";
print_r($acompanhantes9);
echo "</pre>";
echo "</div>";

// Verificar se temos acompanhantes
if (empty($acompanhantes9) && isset($reserva9['qtd_pessoas']) && $reserva9['qtd_pessoas'] > 1) {
    ...
    $numAcompanhantes9 = $reserva9['qtd_pessoas'] - 1;
    for ($i = 0; $i < $numAcompanhantes9; $i++) {
        $acompanhantes9[] = [
...
$_SESSION['acompanhantes'] = $acompanhantes9;
...
$stmtAcompanhantesSimples->bind_param("i", $reserva9_id);
$stmtAcompanhantesSimples->execute();
$resultAcompanhantesSimples = $stmtAcompanhantesSimples->get_result();
$acompanhantes9 = [];

while ($acompanhante9 = $resultAcompanhantesSimples->fetch_assoc()) {
    $acompanhantes9[] = $acompanhante9;
}
...
echo "<pre>SELECT * FROM acompanhantes WHERE reserva_id = {$reserva9_id}</pre>";
...
$queryDebug = "SELECT a.*, s.nm_sexo, v.nm_vinculo 
               ...
               WHERE a.reserva_id = {$reserva9_id}";
...
if (!empty($acompanhantesCompleta)) {
    $acompanhantes9 = $acompanhantesCompleta;
